...
class EAPI
{
    ...
    public function sendRequest($request, $parameters = array())
    {
        ...
        if($this->runProfiler === true) {
            ...
            $profilerLog  = sprintf("%s %s: %s Duration: %s sec. Params: %s\n", date('Y-m-d H:i:s'), $logRand, $request, (time() - $profilerStart), serialize($parameters));
            ...
            // Log response
            $responseLogFile = $logsDir . '/' . sprintf('%s_%s_%s.txt', date('Y.m.d-H.i.s'), $request, $logRand);
    ...
    private function getSessionKey(&$result)
    {
        ...
        global $_SESSION;
        if($_SESSION['EAPISessionKey'][$this->username] && $_SESSION['EAPISessionKeyExpires'][$this->username] > time()) {
            ...
            if(isset($response['records']) && is_array($response['records'][0]) && !empty($response['records'][0]['sessionKey'])) {
                ...
                $_SESSION['EAPISessionKey'][$this->username]        = $response['records'][0]['sessionKey'];
                $_SESSION['EAPISessionKeyExpires'][$this->username] = time() + $response['records'][0]['sessionLength'] - 30;

...
class Erply_Sync_Abstract
{
     ...
     * @return ErplyMapping
     */
    public static function createCategoryMapping($localId, $erplyId, $parentId = null)
    {
        ...
        if($parentId) {
            $mappingObj->setInfo('parent', $parentId);
        }
        if($mappingObj->add()) {
            // Add to cache
     ...
     * @return ErplyMapping
     */
    public static function getParentProductMapping($fieldName, $fieldValue)
    {
        $mapping = ErplyMapping::getMapping('ParentProduct', $fieldName, $fieldValue);
        if(!is_null($mapping)) {
            // Check if valid mapping.
    ...
    public static function logAndReturn($msg, $level = 'ok')
    {
        $output = '';
        ErplyFunctions::log($msg);
        switch($level) {
            default:
            case 'ok':
                $output .= Utils::displayConfirmation($msg);
                break;
            case 'warn':
                $output .= Utils::displayWarning($msg);
                break;
            case 'ferr':
                $msg = 'FATAL ERORR: ' . $msg;
            case 'err':
                $output .= Utils::displayError($msg);
                break;
        }
        return $output;
    }

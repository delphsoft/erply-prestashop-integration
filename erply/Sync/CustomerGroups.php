            ...
            if(!is_null($mappingObj)) {
                ...
                if(!self::prestaGroupHasChanged($mappingObj->getPrestaId()) || $objectPriority == 'ERPLY') {
                    ...
                    if(($resp = self::updatePrestaGroup($prestaGroupObj, $erplyGroupAry, $mappingObj))) {
                        ...
                        $output .= Utils::displayConfirmation('Override Presta Cusomter Group with erply data. Presta  Cusomter Group id: ' . $mappingObj->getPrestaId() . ' name: ' . $erplyGroupAry['name']);
                    }
                }
            } else {
                ...
                if(($resp = self::createPrestaGroup($erplyGroupAry))) {
                    ...
                    $output .= Utils::displayConfirmation('Create Presta Cusomter Group with erply data. Name: ' . $erplyGroupAry['name']);
                }
            }
            ...
            if(($mappingObj = self::getCustomerGroupMapping('local_id', $prestaGroupId))) {
                ...
                if(!self::erplyGroupHasChanged($mappingObj->getErplyId()) || $objectPriority == 'Presta') {
                    ...
                    if(self::updateErplyGroup($prestaGroupObj, $mappingObj)) {
                        $output .= Utils::displayConfirmation('Override erply Customer Group with Presta data. Presta Customer Group id: ' . $mappingObj->getPrestaId());
                    }
                }
            } else {
                ...
                if(($resp = self::createErplyGroup($prestaGroupObj))) {
                    $output .= Utils::displayConfirmation('Create erply Customer Group with Presta data.');
                }
            }

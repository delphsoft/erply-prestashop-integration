...
class Erply_Sync_Orders extends Erply_Sync_Abstract
{
     ...
     */
    public static function createErplyOrder($prestaOrderObj)
    {
     ...
     * @return boolean
     */
    public static function updateErplyOrder($prestaOrderObj, $mappingObj = null)
    {
        if(!is_null($mappingObj)) {
        ...
        // warehouse
        $erplyOrderAry['warehouseID'] = Configuration::get('ERPLY_WAREHOUSE');
        ...
        if(is_array($prestaItemsAry)) {
            foreach($prestaItemsAry as $prestaOrderItemAry) {
                // productID
                $prestaProductMappingObj = self::getProductMapping('local_id', $prestaOrderItemAry['product_attribute_id']);
                if($prestaProductMappingObj) {
                ...
                if(is_null($erplyVatrate))
                    $erplyVatrate = self::getErplyVatrateByRate(0);
        ...
        if(floatval($prestaOrderObj->total_shipping) > 0) {
            $var = $prestaOrderObj->total_shipping;
            // Item name
     ...
     * @return array
     */
    public static function getPrestaChangedOrdersIds()
    {
